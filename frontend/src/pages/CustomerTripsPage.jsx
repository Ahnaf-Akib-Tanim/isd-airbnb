...
import { getBookingsByGuest, cancelBooking } from "../services/bookingService";
...
const STATUS_CONFIG = {
  ...
  CHECKED_IN: { label: "Checked In", color: "#1e40af", bg: "#dbeafe", icon: "🏨" },
...
const CANCELLATION_POLICIES = {
  FLEXIBLE: { label: "Flexible", desc: "Full refund up to 24 hours before check-in", refundPercent: (daysLeft) => daysLeft >= 1 ? 100 : 50 },
  MODERATE: { label: "Moderate", desc: "Full refund if cancelled 5+ days before check-in", refundPercent: (daysLeft) => daysLeft >= 5 ? 100 : daysLeft >= 1 ? 50 : 0 },
  STRICT: { label: "Strict", desc: "50% refund if cancelled 7+ days before check-in", refundPercent: (daysLeft) => daysLeft >= 7 ? 50 : 0 },
};
...
const CustomerTripsPage = () => {
      ...
      await Promise.all(
        hostIds.map(async (id) => {
          try {
            ...
            cache[id] = res.data;
          } catch { cache[id] = { hostDisplayName: "Host", firstName: "Host" }; }
        })
      );
      setHostCache(cache);
    } catch (err) {
  ...
  const categorized = useMemo(() => {
    ...
    bookings.forEach((b) => {
      if (["CANCELLED", "REFUNDED"].includes(b.status)) {
        cancelled.push(b);
      } else if (["COMPLETED"].includes(b.status) || (b.checkOutDate && b.checkOutDate < today)) {
        past.push(b);
      } else {
        current.push(b);
      }
  ...
  const getNights = (b) => b.checkInDate && b.checkOutDate
  ...
  const renderTimeline = (booking) => {
    const events = [];
    if (booking.createdAt) events.push({ date: booking.createdAt, label: "Booking created", icon: "📝" });
    if (booking.status === "CONFIRMED" || booking.status === "CHECKED_IN" || booking.status === "COMPLETED")
      events.push({ date: booking.updatedAt, label: "Confirmed by admin", icon: "✅" });
    if (booking.status === "CHECKED_IN" || booking.status === "COMPLETED")
      events.push({ date: booking.checkInDate, label: "Checked in", icon: "🏨" });
    if (booking.status === "COMPLETED")
      events.push({ date: booking.checkOutDate, label: "Completed", icon: "🎉" });
  ...
  const renderBookingCard = (booking) => {
    ...
    const canPay = booking.paymentStatus === "PAY_LATER" && ["CONFIRMED", "PENDING"].includes(booking.status);
    const refundPct = canCancel ? getRefundPercent(booking) : 0;

    return (
      <div className="ct-card" key={booking.id} onClick={() => navigate(`/booking/${booking.id}`)}>
        <div className="ct-card-header">
          ...
          <div className="ct-card-info">
            ...
            <p className="ct-card-location">📍 {host.area || host.city || host.district || "N/A"}, {host.country || ""}</p>
            <div className="ct-card-dates">
              <span>{booking.checkInDate ? new Date(booking.checkInDate).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) : "—"}</span>
              <span className="ct-arrow">→</span>
              <span>{booking.checkOutDate ? new Date(booking.checkOutDate).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) : "—"}</span>
              <span className="ct-nights">{nights} night{nights !== 1 ? "s" : ""}</span>
            </div>
          </div>
  ...
  return (
    <div className="ct-page">
      <div className="ct-container">
        ...
        {loading ? (
          <div className="ct-loading"><div className="spinner" /><p>Loading your trips...</p></div>
        ) : activeBookings.length === 0 ? (
          <div className="ct-empty">
            <div className="ct-empty-icon">{activeTab === "current" ? "🏖️" : activeTab === "past" ? "📋" : "💤"}</div>
            <h3>No {activeTab} trips</h3>
            <p>{activeTab === "current" ? "You don't have any upcoming trips. Start exploring!" : activeTab === "past" ? "No completed trips yet." : "No cancelled bookings."}</p>
            {activeTab === "current" && (
              <button className="ct-btn ct-btn-explore" onClick={() => navigate("/")}>Explore homes</button>
            )}
          </div>
        ) : (
          <div className="ct-cards">{activeBookings.map(renderBookingCard)}</div>
        )}
      </div>
          ...
          <div className="ct-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="ct-modal-policy">
              ...
              <div className="ct-refund-preview">
                ...
                <strong>{getRefundPercent(cancelModal)}% — ${Math.round((cancelModal.totalPrice || 0) * getRefundPercent(cancelModal) / 100)}</strong>
              </div>
            </div>
            ...
            <div className="ct-modal-actions">
              <button className="ct-btn ct-btn-secondary" onClick={() => setCancelModal(null)}>Keep Booking</button>

...
const ListingDetailsPage = () => {
  ...
  const handleReserve = () => {
    ...
    // Navigate to reservation page with all details
      ...
      <div className="listing-page">
        <div className="listing-container">
          <div className="listing-loading">
            <div className="skeleton-pulse" style={{ width: '60%', height: 32, borderRadius: 8, marginBottom: 24 }} />
            <div className="skeleton-pulse" style={{ width: '100%', height: 400, borderRadius: 12, marginBottom: 32 }} />
            <div style={{ display: 'flex', gap: 60 }}>
              <div style={{ flex: 2 }}>
                <div className="skeleton-pulse" style={{ width: '50%', height: 24, borderRadius: 6, marginBottom: 12 }} />
                <div className="skeleton-pulse" style={{ width: '40%', height: 16, borderRadius: 6, marginBottom: 24 }} />
                <div className="skeleton-pulse" style={{ width: '100%', height: 80, borderRadius: 6 }} />
              </div>
              <div style={{ flex: 1 }}>
                <div className="skeleton-pulse" style={{ width: '100%', height: 300, borderRadius: 12 }} />
              </div>
            </div>
          </div>
        </div>
  ...
  if (!host) return (
    <div className="listing-page">
      <div className="listing-container" style={{ textAlign: 'center', paddingTop: 80 }}>
        <h2>Host not found</h2>
        <p style={{ color: '#717171' }}>This listing may have been removed or is unavailable.</p>
        <button className="reserve-btn" style={{ marginTop: 20, width: 'auto', padding: '12px 32px' }}
          onClick={() => navigate('/')}>
          Back to Home
        </button>
      </div>
  ...
  const nightlyRate = host.nightlyRateUsd || 50;
    ...
    <div className="listing-page">
      <div className="listing-container">
        ...
        <div className="listing-gallery">
          <img
            src={host.hostPortfolioImages?.[selectedImage] || host.hostPortfolioImages?.[0] || host.profileImage}
            alt="Main"
            className="main-img"
          />
          <div className="sub-imgs">
            {host.hostPortfolioImages?.slice(1, 5).map((img, i) => (
              <img
                key={i}
                src={img}
                alt={`Sub ${i}`}
                onClick={() => setSelectedImage(i + 1)}
                style={{ cursor: 'pointer', opacity: selectedImage === i + 1 ? 0.7 : 1 }}
              />
            ))}
          </div>
        </div>
        ...
        <div className="listing-content">
          ...
          <div className="listing-info">
            <h2>
              {host.propertyTypesOffered?.[0] || "Entire place"} hosted by{" "}
              {host.firstName}
            </h2>
            <p className="listing-specs">
              {host.guestCapacity || 2} guests · {host.bedCount || 1} bedroom ·{" "}
              {host.bedCount || 1} bed · 1 bath
            </p>
            ...
            <div className="listing-about">
              ...
              <p>{host.hostAbout || "A beautiful place to stay. Enjoy your visit!"}</p>
            </div>
            ...
            <div className="listing-amenities">
              ...
              <ul className="amenities-grid">
                {(host.offeringHighlights || ["Wifi", "Kitchen", "Free parking"]).map(
                  (h) => (
                    <li key={h} className="amenity-item">
                      <span className="amenity-icon">✓</span> {h}
                    </li>
                  )
                )}
              </ul>
            ...
            {host.latitude && host.longitude && (
              <>
                ...
                <div className="listing-location">
                  ...
                  <p>{[host.district, host.city, host.country].filter(Boolean).join(", ")}</p>
                  <div style={{ height: 300, borderRadius: 12, overflow: 'hidden', marginTop: 16 }}>
                    <MapContainer
                      center={[host.latitude, host.longitude]}
                      zoom={14}
                    ...
                    >
                      ...
                      />
                      <Marker position={[host.latitude, host.longitude]}>
                        <Popup>{host.hostDisplayName || host.firstName}'s place</Popup>
                      </Marker>
                    </MapContainer>
                  </div>
                </div>
          ...
          <div className="booking-card">
            ...
            <div className="guests-input">
              ...
              >
                {Array.from({ length: host.guestCapacity || 4 }, (_, i) => (
            ...
            {nights > 0 && (
              <div className="booking-card__breakdown">
                <div className="breakdown-row">
                  <span>${Math.round(nightlyRate)} × {nights} night{nights > 1 ? 's' : ''}</span>

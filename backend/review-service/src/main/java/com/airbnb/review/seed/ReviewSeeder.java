...
public class ReviewSeeder implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        try {
            log.info("Fetching valid hosts from user-service at {}", userServiceUrl);
            List<Map<String, Object>> hosts = restTemplate.getForObject(
                userServiceUrl + "/api/users/hosts/suggestions?limit=100", 
                List.class
            );
            
            if (hosts != null && !hosts.isEmpty()) {
                validHostIds = hosts.stream()
                                    .map(h -> (String) h.get("userId"))

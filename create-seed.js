...
const hostPortfolioImagesOptions = [
  ["https://images.unsplash.com/photo-1502672260266-1c1ef2d93688?auto=format&fit=crop&w=800&q=80", "https://images.unsplash.com/photo-1512917774080-9991f1c4c750?auto=format&fit=crop&w=800&q=80", "https://images.unsplash.com/photo-1501183638710-841dd1904471?auto=format&fit=crop&w=800&q=80"],
  ["https://images.unsplash.com/photo-1502672260266-1c1ef2d93688?auto=format&fit=crop&w=800&q=80", "https://images.unsplash.com/photo-1566073771259-6a8506099945?auto=format&fit=crop&w=800&q=80", "https://images.unsplash.com/photo-1580587771525-78b9dba3b914?auto=format&fit=crop&w=800&q=80"],
...
const reviewsTexts = [
  ...
  "Nice place, decent internet speed for working.",
  "Beautifully decorated and close to all the main attractions!"
];
...
for (let i = 0; i < numHosts; i++) {
  ...
  const hostPic = getRandom(guestImages); // using guest images as host profiles too for simplicity
  ...
  const reviewCount = getRandInt(10, 50);
  const avgRating = getRandFloat(4.2, 5.0);

  hosts.push({
    "_class": "com.airbnb.user.model.User",
    "userId": hostId,
    "email": `host${i}_${hostId.substring(0,5)}@example.com`,
    "password": "$2b$12$qfvtKxBnraUKMWRTajeKtuqh861kyso7Wb6AtYPairQltIsYIKWuW", // valid bcrypted hash
    "firstName": firstName,
    "lastName": lastName,
  ...
  // Adding 3-5 reviews for each host
  const numPropsToReview = hostedProperties.length;
  for (let k = 0; k < numPropsToReview; k++) {
...
if (typeof db !== 'undefined') {
  try {
    print("Inserting 150 hosts...");
    db.users.insertMany(hosts); // hosts collection doesn't exist? Wait, they used db.hosts earlier but maybe it was users. The code in seed-hosts-multiple-properties used db.hosts!
    const resH = db.hosts.insertMany(hosts);
    print("Hosts inserted:");
    print(resH.insertedCount);
    
    print("Inserting " + reviews.length + " reviews...");
    // make sure collection reviews exists 
    const resR = db.reviews.insertMany(reviews.map(r => {
      // transform date strings back to ISODate if applicable, but mongodb driver handles it loosely if it's string.
      r.createdAt = new ISODate(r.createdAt);
      r.updatedAt = new ISODate(r.updatedAt);
      return r;
    }));
    print("Reviews inserted:");
    print(resR.insertedCount);
...
fs.writeFileSync('c:/Buet/CSE 326 ISD/Airbnb project/execute-seed.js', fileContent);
console.log('execute-seed.js created successfully!');
